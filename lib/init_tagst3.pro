...
pro init_tagsT3,num_pts,hdata,metadata,num_tags
...
hdata = { hours_hirate:       fltarr(num_pts), $    ;0  100
          ...
          pelev:              fltarr(num_pts), $	;60 205
          thetae:             fltarr(num_pts), $	;61 206
          sat_mix_ratio:      fltarr(num_pts), $	;62 207
          dzdt:               fltarr(num_pts), $	;63 208
          indic_as:           fltarr(num_pts), $	;64 209
          updraft_uncor:      fltarr(num_pts), $	;65 210
          calc_tas:           fltarr(num_pts), $	;66 211
          updrft_cor_factor:  fltarr(num_pts), $	;67 212
          cooper_updraft:     fltarr(num_pts), $	;68 213
          kopp_updraft:       fltarr(num_pts), $	;69 214
          geopot_alt:         fltarr(num_pts), $	;70 215
          turb:               fltarr(num_pts), $	;71 216
          densmid:            fltarr(num_pts), $	;72 217
          fssp_mix_ratio:     fltarr(num_pts), $	;73 219
          hail_mix_ratio:     fltarr(num_pts), $	;74 220
          ez_aircraft:		  fltarr(num_pts), $	;75 260
          ey_aircraft:        fltarr(num_pts), $	;76 261
          ex_aircraft:        fltarr(num_pts), $	;77 262
          ez_path:            fltarr(num_pts), $	;78 263
          ey_path:    		  fltarr(num_pts), $	;79 264
          ex_path:    		  fltarr(num_pts), $	;80 265
          ez_earth:           fltarr(num_pts), $	;81 266
          ey_earth:           fltarr(num_pts), $	;82 267
          ex_earth:           fltarr(num_pts), $	;83 268
          eq_aircraft:        fltarr(num_pts), $	;84 269
          eq_temp:            fltarr(num_pts), $	;85 270
          accel_x:            fltarr(num_pts), $	;86 290
          accel_y:            fltarr(num_pts), $	;87 291
          accel_z:            fltarr(num_pts), $ 	;88 292
          hvps_house1:        fltarr(num_pts), $	;89 671
          hvps_house2:        fltarr(num_pts), $	;90 672
          hvps_house3:        fltarr(num_pts), $	;91 673
          hvps_house4:        fltarr(num_pts), $	;92 674
          hvps_house5:        fltarr(num_pts), $	;93 675
          hvps_house6:        fltarr(num_pts), $	;94 676
          hvps_house7:        fltarr(num_pts), $	;95 677
          hvps_house8:        fltarr(num_pts),$ 	;96 678
		  dyn_diff:			  fltarr(num_pts), $    ;97 900
		  stat_diff:          fltarr(num_pts), $	;98 901
          temp_diff:          fltarr(num_pts), $	;99 902
          heading_gps_trk_angl:fltarr(num_pts), $	;100 903
          accelZ_accel:        fltarr(num_pts), $	;101 904
          NCARTAS_CalcTAS:     fltarr(num_pts), $	;102 905
          NCARTAS_GPSgrdSpd:   fltarr(num_pts), $	;103 906
          GPSalt_Pelev:        fltarr(num_pts), $ 	;104 907
          heading:            fltarr(num_pts), $      ;105 990
          king_lwc:            fltarr(num_pts)} ; tag 125 in 1995
...
metadata(81) = { md , 266 , 'Ez Earth Ref'       , 'EzER' , 'kV/m'    , 'f5.1' ,   -5. ,    5. }
metadata(82) = { md , 267 , 'Ey Earth Ref'       , 'EyFR' , 'kV/m'    , 'f5.1' ,   -5. ,    5. }
metadata(83) = { md , 268 , 'Ex Earth Ref'       , 'ExFR' , 'kV/m'    , 'f5.1' ,   -5. ,    5. }
...
metadata(105) = { md , 990  , 'Heading   '               , 'Hdg'  , 'deg'     , 'f6.1' ,    0. ,  360. }
metadata(106) = { md , 991 , 'King Probe LW'       , 'KLW', 'g/m3'    , 'f4.1' ,    0. ,    4. }
end
