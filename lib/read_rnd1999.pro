...
;- read_rnd1995.pro
;-
;- Modified program read_rnd.pro to accomodate for the 1995 HVPS data
;- LAST MODIFIED: Jan, 2002
; last modified: March 19, 2008
;----------------------------------------------------------------------

pro read_rnd1999,fn,num_tags,tags,tag_ind,data,num, hrs, hms, err
...
tag_ind = intarr(500)  ; yr1999
tag_ind(*) = -1
for i=0,num_tags-1 do begin
 tem = tags(i)
 ;print,'tem for tag_ind is: ', tem   ; 6/22/00
 tag_ind(tem) = i
 ;print,'i,tem,tag_ind(tem):',i,tem,tag_ind(tem)
endfor
;stop
